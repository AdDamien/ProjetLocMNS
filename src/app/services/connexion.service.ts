...
export class ConnexionService {
  ...
  updateUserConnected() {
    ...
    if (jwt != null) {
      ...
      // donc quand tu t'es connectes ici ça va nous permettre de connaitre celui qui est connecté; 

      const data = jwt.split('.')[1]; // donc on recupère l'index 1
      const json = window.atob(data); // on en fait du texte 
      const donneesUtilisateur = JSON.parse(json); // ensuite on en fait du JSON hein,
      ...
      // et donc ici on vient vérifier par le biais d'un observale s'il est connecté comme ceci
